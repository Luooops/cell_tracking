# Cellpose GUI:标注 Ground Truth 与模型微调(macOS)

一份指南,包含 **(1)** 用 Cellpose GUI 制作 ground-truth(GT)实例 mask,以及
**(2)** 用 human-in-the-loop 流程微调 Cellpose 模型。

> 基于 **Cellpose v4.2.0**。注意:macOS没有 NVIDIA/CUDA GPU,所以 Cellpose 跑在 CPU 上。能用,只是比 CUDA 机器慢。
...
### 1. 安装 Miniconda
从 <https://docs.conda.io/en/latest/miniconda.html> 下载 **macOS** 安装包:
- **Apple 芯片(M1/M2/M3)**:选 `arm64` 版本
- **Intel Mac**:选 `x86_64` 版本

运行安装(双击 `.pkg`,或在 Terminal 里跑 `.sh`)。
...
```
`[gui]` 会装图形界面(PyQt、pyqtgraph);pip 同时会装 PyTorch(Mac 上是 CPU/MPS 版)。

### 4. 验证是否安装成功
```bash
python -c "import cellpose, torch; print(cellpose.__version__)"
```
(Mac 上 `torch.cuda.is_available()` 是 `False`,正常 —— 它用 CPU。)
...
# ---- 改这两个路径 ----
IN_DIR = "/Users/你的用户名/存放灰度图的文件夹"      
OUT_DIR = "/Users/你的用户名/转换后RGB的输出文件夹"   
...
> Mac 的快捷键可能和 Windows 不同。可以在 **Help 菜单 → key/mouse commands** 确认。

### 第 6 步:保存
按 **`Cmd + S`**。会在**原图旁边**生成一个 `<图片名>_seg.npy` 文件，里面就是修正后的
...
1. 加载一张【新图】(模型没训练过的)
2. 跑当前模型  →  它给出预测 mask
3. 改它的错     (Cmd+点=删, 右键拖=加)
4. Cmd+S 存修正后的 _seg.npy(存在图片旁边)
5. 攒几张后,重新训练(见下)
6. 新的(更准的)模型去预测下一批图
7. 重复 —— 每轮改得越来越少
```
